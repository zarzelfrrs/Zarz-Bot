...
const handler = async (m, { conn, text, command }) => {
  ...
  try {
    const res = await fetch(`https://api.siputzx.my.id/api/ai/claude-sonnet-37?content=${encodeURIComponent(text)}`);
...
handler.help = ['claude37 <teks>'];
handler.tags = ['ai'];
handler.command = /^claude37$/i;
