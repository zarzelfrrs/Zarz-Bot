...
const handler = async (m, { conn, text, command }) => {
  ...
  try {
    const res = await fetch(`https://api.siputzx.my.id/api/ai/meta-llama-33-70B-instruct-turbo?content=${encodeURIComponent(text)}`);
...
handler.help = ['llama70b <teks>'];
handler.tags = ['ai'];
handler.command = /^llama70b$/i;
