  ...
  let bang = m.key.id
	...
	try {
		...
		if (/webp/g.test(mime)) {
			...
			stiker = await addExif(img, global?.info?.packname ?? m.name ?? '', global.info.author || '')
		} else if (/image/g.test(mime)) {
			let img = await q.download?.()
			stiker = await createSticker(img, false, global?.info?.packname ?? m.name ?? '', global.info.author)
		} else if (/video/g.test(mime)) {
		//	if ((q.msg || q).seconds > 10) throw 'Max 10 seconds!'
			let img = await q.download?.()
			stiker = await mp4ToWebp(img, { packname: global?.info?.packname ?? m.name ?? '', author: global.info.author })
